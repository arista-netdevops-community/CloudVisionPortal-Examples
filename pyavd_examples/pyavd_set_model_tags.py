...
async def deploy_to_cv(
    ...
    try:
        async with CVClient(servers=cloudvision._servers, token=cloudvision._token, verify_certs=cloudvision._verify_certs) as cv_client:
            ...
            existing_tags = await cv_client.get_tag_assignments(workspace_id="", creator_type="system",element_type="device")
            existing_tags_tuples = [("sw_" + tag.key.label, tag.key.value, tag.key.device_id, None) for tag in existing_tags if tag.key.label == "model"]
            new_tags = [("sw_" + tag.key.label, tag.key.value) for tag in existing_tags if tag.key.label == "model"]
            ...
            try:
                ...
                logging.info("Deploying tags...")
                await cv_client.set_tags(
                    tags=new_tags,
                    workspace_id=result.workspace.id,
                    element_type="device"
                )
                logging.info("Assigning tags to devices...")
                await cv_client.set_tag_assignments(
                    tag_assignments=existing_tags_tuples,
                    workspace_id=result.workspace.id,
                    element_type="device"
                )
...
if __name__ == "__main__":
    ...
    asyncio.run(deploy_to_cv(cloudvision=CloudVision))
