# EST Enrollment using studio
 EST protocol allowing simple automatic Enrollment and re-Enrollment/renewal of certications.
 The protocol is using client/server model, Arista's switch can act as EST client and AGNI as EST server.
 This example studio and custom actions package can be used in CVP/Cvaas to generate the necessary EOS configuration for EST protocol.
...
# Prerequisite for using the EST Studio:
 ...
 4. Use the certificate_management package to push both EST & Radsec Root CA certificate into each switch.

# How to use EST studio:
1. Add The desired devices into the device selection inside the EST studio.
